...
export const generalController = {
	uploadGenerals: async (req: Request, res: Response) => {
		try {
			if (!req.file) {
			...
			const generals = await Promise.all(
				data.map(async (row: any) => {
					...
					try {
						return await prisma.general.upsert({
							where: { id },
							update: {
								state: String(row.state),
								substate: String(row.substate),
			...
			res.status(201).json({
				mensaje: `${generals.length} registros generales importados con éxito`,
			});
		} catch (error) {
			console.error("Error al importar registros generales:", error);
